...
from .chain import call_llm

# vectorstore 위치: env var 우선, 기본값은 sandbox 앱 경로
_DEFAULT_VS = Path(__file__).resolve().parent.parent.parent / "01_notebooks" / "99_sandbox" / "streamlit_dasol"
BASE_DIR  = Path(os.environ.get("VECTORSTORE_DIR", str(_DEFAULT_VS)))
...
def process_q1(session: dict, answer: str) -> dict:
    ...
    session["candidates"] = candidates[:8]
